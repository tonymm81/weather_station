...
- Fixing also the autorestart issue, when all the esp:s are not responsing, the weatherstation is not resetting them.

## version 139

- Lets wake up this old project. I change the hardware side a littlebit. Rasbin is no longer in mirror. I change it in 8 inch  display. I have to also adjust the graphics.
- Some times WS is failing during the longer uptime so I have to add the error handling and logging to program to see, what is failing and why.
- I have to also update the graphics, besause UI is now litlebit messy.

 - I have to add in table_project the function, what shutdown this device via mqtt
shutdown is not easy to develope, because I have to some how wait, that weatherstation first shutdown the esp broadlink outlets, and then it shutdown itself
and after that we can shutdown the table project.

- Now the program has new ui layout and it has also logging in broadlink_switch and weatherstation and connection.py files.
